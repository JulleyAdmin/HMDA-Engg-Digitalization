...
const FilterControl: React.FC<FilterControlProps> = ({
  ...
  const renderControl = () => {
    switch (config.type) {
      ...
      case 'multiselect':
        const selectedValues = filter?.value && Array.isArray(filter.value) ? filter.value as (string | number)[] : [];
        return (
          <FormControl fullWidth size="small">
            <InputLabel>{config.label}</InputLabel>
            <Select
      ...
      case 'range':
        const rangeValue = filter?.value && Array.isArray(filter.value) ? filter.value as number[] : [0, 100];
        return (
          <Box>
